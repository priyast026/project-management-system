...
export const createProject = async (req, res) => {
    try {
        ...
        const {workspaceId, description, name, status, start_date, end_date, team_members, team_lead, progress,
             priority} = req.body;
             ...
             if(!workspace.members.some((member)=>member.userId === userId && member.role === "ADMIN")){
                return res.status(404).json({ message: "You don't have permission to create projects in this workspace"});
             }
...
if (team_members?.length > 0) {
    workspace.members.forEach((member) => {
        if (
            ...
            !membersToAdd.includes(member.user.id)
        ) {
            membersToAdd.push(member.user.id);
        }
    });
...
export const updateProject = async (req, res) => {
    try {
        const{userId} = await req.auth();
        const {id , workspaceId, description, name, status, start_date, end_date, team_members, team_lead, progress,
             priority } = req.body;
